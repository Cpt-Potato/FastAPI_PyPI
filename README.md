...
```
* Перейти по адресу 127.0.0.1:8000 или 127.0.0.1:8000/docs
### Также можно подключиться к локальной PostgreSQL 
* Изменить DATABASE_URL в файле .env.dev \
(заменить localhost на host.docker.internal для подключения к локальной БД из Docker)
